...
for files in [1, 2, 3, 4, 5, 6, 7, 8]:
    for curGt in gtType:
        ...
        curData_Sep = []
        if os.path.exists(os.path.join(pairSaveDir, 'a-s8' + '-' + curGt+'-'+'sample'+'-'+ str(files))):
            fileIn = open(os.path.join(pairSaveDir, 'a-s8' + '-' + curGt+'-'+'sample'+'-'+str(files)))
